...
export async function POST(request: Request) {
  ...
  try {
    ...
    const found = await payload.find({ collection: 'posts', where: { slug: { equals: input.slug } }, limit: 1, depth: 0, overrideAccess: true })
    ...
    const data = { title: input.title, slug: input.slug, excerpt: input.excerpt || undefined, content, status: 'draft' as const, seoTitle: input.title, seoDescription: input.excerpt || undefined }
