...
export default function CmsArticlesPage() {
  ...
  return <CmsDashboardShell activeKey="articles" title="Quản lý Bài viết" description="Soạn, biên tập và phân phối bài viết đến đúng chuyên mục, vị trí hiển thị và đối tượng độc giả.">
    ...
    <article className="panel">
      ...
      <div className="cms-link-grid">{newsSignalCards.map((signal) => <article key={signal.key} className={signal.key === selectedSignal ? 'cms-link-card cms-link-card-active' : 'cms-link-card'}><strong>{signal.label}</strong><span>{signal.value}</span><span>Chuyên mục phù hợp: {signal.categories.join(', ')}</span><div className="cms-inline-actions"><button type="button" className="button-secondary" onClick={() => applySignal(signal)}>Áp dụng cho bài viết</button><Link className="cms-table-link" href={`/tin-tuc?signal=${signal.key}`}>Xem trên trang tin</Link></div></article>)}</div>
    </article>
    ...
    <article className="panel cms-article-editor-panel">
      ...
      <form className="form-shell cms-embedded-form">
        <div className="cms-article-meta-grid"><div className="field"><label htmlFor="postTitle">Tiêu đề bài viết</label><input id="postTitle" placeholder="Headline nổi bật cho nightlife / entertainment" /></div><div className="field"><label htmlFor="postSlug">Slug / đường dẫn</label><input id="postSlug" placeholder="nightlife-weekend-saigon" /></div><div className="field"><label htmlFor="postCategory">Chuyên mục</label><select id="postCategory">{articleCategories.map((category) => <option key={category}>{category}</option>)}</select></div><div className="field"><label htmlFor="postSeries">Chuyên đề</label><select id="postSeries">{seriesList.map((series) => <option key={series.title}>{series.title}</option>)}<option>Không gắn chuyên đề</option></select></div><div className="field"><label htmlFor="postStatus">Trạng thái</label><select id="postStatus"><option>Nháp</option><option>Chờ media</option><option>Chờ duyệt</option><option>Xuất bản</option><option>Lên lịch</option></select></div><div className="field"><label htmlFor="postPlacement">Vị trí hiển thị</label><select id="postPlacement" value={postPlacement} onChange={(event) => setPostPlacement(event.currentTarget.value)}>{cmsNewsPlacementOptions.map((placement) => <option key={placement}>{placement}</option>)}</select><span className="cms-field-hint">Đang áp dụng: {activeSignal.label}</span></div></div>
        <div className="field"><label htmlFor="postExcerpt">Tóm tắt</label><textarea id="postExcerpt" placeholder="Viết 2-3 câu ngắn cho card, SEO intro và feed tin tức..." /></div>
