...
import { cmsMusicRows, cmsMusicTabOptions, type CmsMusicTabKey } from '@/lib/cms-dashboard-data'
...
const getTabHref = (tab: CmsMusicTabKey) => tab === 'all' ? '/cms/dashboard/music' : `/cms/dashboard/music?tab=${tab}`
const getFilteredMusic = (tab: CmsMusicTabKey) => tab === 'all' ? cmsMusicRows : cmsMusicRows.filter((item) => item.type === tab)
...
export default async function CmsMusicPage({ searchParams }: { searchParams: Promise<{ tab?: string; page?: string }> }) {
  const params = await searchParams
  const activeTab = params.tab && validTabs.has(params.tab as CmsMusicTabKey) ? params.tab as CmsMusicTabKey : 'all'
  const filteredMusic = getFilteredMusic(activeTab)
  const totalPages = Math.max(1, Math.ceil(filteredMusic.length / CMS_LIST_PAGE_SIZE))
  const page = Math.min(totalPages, Math.max(1, Number.parseInt(params.page ?? '1', 10) || 1))
  const visibleMusic = filteredMusic.slice((page - 1) * CMS_LIST_PAGE_SIZE, page * CMS_LIST_PAGE_SIZE)
    ...
    <CmsDashboardShell activeKey="music" title="Quản lý Music">
      <section className="cms-panel">
        ...
        <nav className="cms-tab-row" aria-label="Lọc thư viện nhạc">
          {cmsMusicTabOptions.map((option) => <Link key={option.key} className={option.key === activeTab ? 'cms-tab-button cms-tab-button-active' : 'cms-tab-button'} href={getTabHref(option.key)}>{repairVietnameseText(option.label)}</Link>)}
        </nav>
      </section>
      ...
      <section className="cms-panel">
        ...
        <CmsListPagination page={page} totalItems={filteredMusic.length} baseHref={getTabHref(activeTab)} />
      </section>
    </CmsDashboardShell>
  )
}
