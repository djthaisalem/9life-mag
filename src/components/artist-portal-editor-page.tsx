...
export function ArtistPortalEditorPage({ section }: ArtistPortalEditorPageProps) {
  ...
  return (
    <>
      {isDraftPreviewOpen ? <div className="music-visualizer-overlay" role="dialog" aria-modal="true" aria-label="Bản xem trước nội dung nghệ sĩ"><section className="music-visualizer-shell artist-dashboard-panel artist-profile-draft-preview"><button type="button" className="music-visualizer-close" onClick={() => setIsDraftPreviewOpen(false)}>Đóng</button><p className="section-eyebrow">{section.key === 'profile' ? 'Bản xem trước hồ sơ' : `Bản xem trước ${text(section.title)}`}</p>{section.key === 'profile' && previewValue('coverUpload') ? <img className="artist-profile-draft-cover" src={previewValue('coverUpload')} alt="Cover bản nháp" /> : null}<div className="artist-profile-draft-hero"><div className="artist-profile-draft-avatar">{previewValue('portraitUpload') ? <img src={previewValue('portraitUpload')} alt="" /> : <span>{(profileBasics.artistName || 'N').slice(0, 1)}</span>}</div><div><h2>{profileBasics.artistName || 'Nghệ sĩ chưa đặt tên'}</h2><strong>{section.key === 'profile' ? (profileBasics.headline || 'Chưa có câu giới thiệu nổi bật') : (previewValue('trackTitle') || previewValue('videoTitle') || previewValue('articleTitle') || 'Nội dung chưa đặt tiêu đề')}</strong><p>{section.key === 'profile' ? (profileBasics.shortBio || 'Chưa có giới thiệu ngắn. Hãy bổ sung để người xem hiểu phong cách và kinh nghiệm của bạn.') : 'Đây là bản xem nháp riêng tư trước khi nội dung được gửi Admin kiểm tra.'}</p><span className="cms-status-chip">{profileStatus === 'published' ? 'Hồ sơ đã duyệt' : 'Bản nháp riêng tư'}</span></div></div><div className="artist-dashboard-module-grid"><article className="artist-dashboard-module-card"><strong>Thông tin nghề nghiệp</strong><p>{previewValue('genres') || 'Chưa có phong cách / dòng nhạc'}</p><p>{previewValue('city') || 'Chưa có tỉnh thành hoạt động'}</p></article><article className="artist-dashboard-module-card"><strong>Booking</strong><p>{previewValue('bookingRate') || 'Chưa công khai mức giá booking'}</p><p>{previewValue('availability') || 'Chưa cập nhật tình trạng nhận show'}</p></article><article className="artist-dashboard-module-card"><strong>Kinh nghiệm</strong><p>{previewValue('workExperience') || 'Chưa thêm kinh nghiệm làm việc'}</p></article><article className="artist-dashboard-module-card"><strong>Music / media</strong><p>{previewValue('trackTitle') || previewValue('playlistName') || 'Chưa có nội dung âm nhạc'}</p><p>{previewValue('sourceUrl') || previewValue('videoUrl') || previewValue('videoTitle') || 'Chưa có link music hoặc video'}</p></article></div><article className="artist-dashboard-panel"><p className="section-eyebrow">Giới thiệu chi tiết</p><p>{previewValue('longBio') || previewValue('description') || 'Phần này sẽ hiện khi bạn bổ sung giới thiệu chi tiết.'}</p></article><p className="artist-editor-save-feedback">Các dòng “chưa có” cũng sẽ là danh sách để bạn và Admin biết cần bổ sung trước khi duyệt công khai.</p></section></div> : null}
