...
export function ArtistPortalEditorPage({ section }: ArtistPortalEditorPageProps) {
  ...
  const profileMissing = [
    !profileBasics.artistName?.trim() && 'Tên nghệ sĩ',
    !profileBasics.headline?.trim() && 'Câu giới thiệu nổi bật',
    !profileBasics.shortBio?.trim() && 'Giới thiệu ngắn',
    !profileBasics.primaryRole?.trim() && 'Vai trò chính',
    !previewValue('portraitUpload') && 'Ảnh chân dung',
    !previewValue('coverUpload') && 'Ảnh cover ngang',
    !previewValue('longBio') && 'Giới thiệu chi tiết',
    !previewValue('workExperience') && 'Kinh nghiệm làm việc',
    !previewValue('sourceUrl') && 'Link music',
    !previewValue('videoUrl') && 'Link video',
  ...
  return (
    <>
      {isDraftPreviewOpen ? <div className="music-visualizer-overlay" role="dialog" aria-modal="true" aria-label="Bản xem trước nội dung nghệ sĩ"><section className="music-visualizer-shell artist-profile-page artist-profile-draft-preview"><button type="button" className="music-visualizer-close" onClick={() => setIsDraftPreviewOpen(false)}>Đóng</button><p className="section-eyebrow">Bản xem trước profile public</p><section className="artist-profile-hero artist-profile-draft-hero-public">{previewValue('coverUpload') ? <img className="artist-profile-hero-image" src={previewValue('coverUpload')} alt="Cover bản nháp" /> : null}<div className="artist-profile-hero-overlay" /><div className="artist-profile-hero-inner"><div className="artist-profile-hero-copy"><div className="tag-row"><span className="pill">{profileBasics.primaryRole || 'Chưa chọn vai trò'}</span><span className="pill">{previewValue('city') || 'Chưa có tỉnh thành'}</span></div><h1>{profileBasics.artistName || 'Nghệ sĩ chưa đặt tên'}</h1><p className="artist-profile-lead">{profileBasics.shortBio || 'Chưa có giới thiệu ngắn. Hãy bổ sung để người xem hiểu phong cách và kinh nghiệm của bạn.'}</p></div><div className="artist-profile-draft-avatar">{previewValue('portraitUpload') ? <img src={previewValue('portraitUpload')} alt="" /> : <span>{(profileBasics.artistName || 'N').slice(0, 1)}</span>}</div></div></section><div className="artist-profile-grid-main"><div className="artist-profile-main"><article className="artist-panel"><div className="artist-panel-head"><div><p className="section-eyebrow">Overview</p><h2>Thông tin nổi bật</h2></div></div><div className="artist-overview-grid"><article><strong>{previewValue('city') || 'Chưa cập nhật'}</strong><span>Khu vực hoạt động chính</span></article><article><strong>{previewValue('availability') || 'Chưa cập nhật'}</strong><span>Tình trạng nhận show</span></article><article><strong>{previewValue('bookingRate') || 'Chưa cập nhật'}</strong><span>Mức giá tham khảo</span></article></div></article><article className="artist-panel"><div className="artist-panel-head"><div><p className="section-eyebrow">Professional Background</p><h2>Kinh nghiệm làm việc</h2></div></div><p>{previewValue('workExperience') || 'Chưa thêm kinh nghiệm làm việc.'}</p></article><article className="artist-panel"><div className="artist-panel-head"><div><p className="section-eyebrow">Artist Story</p><h2>Giới thiệu về nghệ sĩ</h2></div></div><p>{previewValue('longBio') || 'Chưa có giới thiệu chi tiết.'}</p></article><article className="artist-panel"><div className="artist-panel-head"><div><p className="section-eyebrow">Listen</p><h2>Nhạc nổi bật có thể phát ngay</h2></div></div><strong>{previewValue('trackTitle') || previewValue('playlistName') || 'Chưa có track hoặc mixset'}</strong>{previewValue('sourceUrl') ? <ArtistMediaEmbedPreview url={previewValue('sourceUrl')} /> : <p>Chưa thêm link phát nhạc.</p>}</article><article className="artist-panel"><div className="artist-panel-head"><div><p className="section-eyebrow">Media Showcase</p><h2>Video nổi bật</h2></div></div><strong>{previewValue('videoTitle') || 'Chưa có video'}</strong>{previewValue('videoUrl') ? <ArtistMediaEmbedPreview url={previewValue('videoUrl')} /> : <p>Chưa thêm link video.</p>}</article><article className="artist-panel"><div className="artist-panel-head"><div><p className="section-eyebrow">Editorial</p><h2>Bài viết và spotlight</h2></div></div><strong>{previewValue('articleTitle') || 'Chưa có bài spotlight'}</strong><p>{previewValue('articleExcerpt') || previewValue('editorialNote') || 'Chưa có nội dung editorial.'}</p></article></div><aside className="artist-profile-side"><article className="artist-panel"><div className="artist-side-portrait">{previewValue('portraitUpload') ? <img src={previewValue('portraitUpload')} alt="" /> : <span>Chưa có ảnh chân dung</span>}</div><div className="artist-side-details"><strong>{previewValue('genres') || 'Chưa có phong cách / dòng nhạc'}</strong></div></article><article className="artist-panel"><div className="artist-panel-head"><div><p className="section-eyebrow">Booking Essentials</p><h2>Thông tin đối tác</h2></div></div><p>{previewValue('bookingNotes') || 'Chưa có ghi chú booking.'}</p><p>{previewValue('basicRider') || 'Chưa có rider cơ bản.'}</p></article></aside></div><p className="artist-editor-save-feedback">Đây là bố cục profile public. Các block “chưa có” sẽ không hiện ở site chính sau khi duyệt, nhưng vẫn giữ trong preview để bạn biết cần bổ sung.</p></section></div> : null}
