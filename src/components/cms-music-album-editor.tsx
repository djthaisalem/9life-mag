...
async function cropCover(file: File) {
  ...
  try {
    const image = await new Promise<HTMLImageElement>((resolve, reject) => { const element = new Image(); element.onload = () => resolve(element); element.onerror = () => reject(new Error('Không thể đọc ảnh cover.')); element.src = source })
...
export function CmsMusicAlbumEditor({ albumId, initialAlbum, initialTrackIds, tracks }: { albumId: string; initialAlbum: Album; initialTrackIds: string[]; tracks: Track[] }) {
  ...
  async function save() {
    ...
    try {
      const response = await fetch(`/api/cms/music/albums/${encodeURIComponent(albumId)}`, {
        ...
        body: JSON.stringify({ title: album.title, description: album.description, isPublic: album.isPublic, displayMap: album.isPublic ? 'Music - Album / release' : '', coverDataUrl: coverDataUrl || undefined, trackIds: selected }),
      })
  ...
  return <div className="artist-album-track-picker">
    ...
    <div className="field"><label>Mô tả Album</label><textarea value={album.description} onChange={(event) => setAlbum((current) => ({ ...current, description: event.currentTarget.value }))} /></div>
    <div className="field"><label>Ảnh cover Album</label><input type="file" accept="image/jpeg,image/png,image/webp" onChange={async (event: ChangeEvent<HTMLInputElement>) => { const file = event.currentTarget.files?.[0]; if (file) setCoverDataUrl(await cropCover(file)) }} />{coverDataUrl || album.coverUrl ? <img className="cms-album-cover-preview" src={coverDataUrl || album.coverUrl} alt="Cover Album" /> : null}</div>
    ...
    <div className="cms-inline-actions"><button type="button" className="button" disabled={isSaving || !album.title.trim() || !selected.length} onClick={() => void save()}>{isSaving ? 'Đang lưu...' : 'Lưu thay đổi Album'}</button></div>
