...
export function CmsMusicUploadForm({ artists, genres, albums }: { artists: ArtistOption[]; genres: GenreOption[]; albums: AlbumOption[] }) {
  ...
  return (
    <form className="form-shell cms-embedded-form" onSubmit={handleSubmit}>
      ...
      {result ? <div className="cms-security-panel"><strong>Đã tạo track nháp</strong><p>Mã quản lý: {result.musicCode}</p><p>Bản phát: {result.previewKey}</p><p>Download: {result.masterKey}</p><p>Cover mặc định: {result.coverR2Key}</p></div> : null}
    </form>
  )
}
