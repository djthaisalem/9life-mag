...
export function CmsNotificationCenter() {
  ...
  useEffect(() => { void fetch('/api/cms/portal-notifications', { cache: 'no-store', credentials: 'include', headers: capability ? { Authorization: `Bearer ${capability}` } : undefined }).then(async (response) => ({ response, body: await response.json() as { ok: boolean; notifications?: Notice[] } })).then(({ response, body }) => { if (response.ok && body.ok) setItems(body.notifications ?? []) }).catch(() => undefined) }, [capability])
  const unread = items.filter((item) => !item.isRead).length
  async function read(item: Notice) { if (!item.isRead) { await fetch('/api/cms/portal-notifications', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json', ...(capability ? { Authorization: `Bearer ${capability}` } : {}) }, body: JSON.stringify({ id: item.id }) }); setItems((current) => current.map((row) => row.id === item.id ? { ...row, isRead: true } : row)) } setOpen(false) }
