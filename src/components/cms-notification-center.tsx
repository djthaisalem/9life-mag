...
export function CmsNotificationCenter() {
  ...
  useEffect(() => {
    const loadNotifications = () => {
      void fetch('/api/cms/portal-notifications', {
        cache: 'no-store',
        credentials: 'include',
        headers: capability ? { Authorization: `Bearer ${capability}` } : undefined,
      })
        .then(async (response) => ({ response, body: await response.json() as { ok: boolean; notifications?: Notice[] } }))
        .then(({ response, body }) => {
          if (response.ok && body.ok) setItems(body.notifications ?? [])
        })
        .catch(() => undefined)
    }
