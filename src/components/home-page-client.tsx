...
export default function HomePageClient({ initialArticles = [] }: HomePageClientProps) {
  const initialPublishedNews = initialArticles
    .filter((article) => article.slug && article.title)
    .map((article) => articleToHomeNewsItem(article))
  const initialFeaturedSlides = initialArticles
  ...
  const [homeNewsItems, setHomeNewsItems] = useState<HomeNewsItem[]>(initialPublishedNews)
  const [homeFeaturedSlides, setHomeFeaturedSlides] = useState<HomeFeaturedSlide[]>(initialFeaturedSlides)
        ...
        <div className="ranking-board-grid">
          <div className="top-ranking top-ranking-tiered">
            {featuredItems.map((item, index) => (
              ...
              >
                {index === 0 ? <img src={item.image} alt="" aria-hidden="true" loading="lazy" decoding="async" className="ranking-podium-fill" /> : null}
                <div className="ranking-podium-content">
                  ...
                  <img src={item.image} alt={item.name} loading="lazy" decoding="async" className="ranking-avatar" />
                  <h3>{item.name}</h3>
          ...
          <div className="ranking-list ranking-list-compact">
            {listItems.map((item) => (
              <div key={item.id} className="ranking-row">
                ...
                <img src={item.image} alt={item.name} loading="lazy" decoding="async" className="ranking-list-avatar" />
            ...
            <div className="ranking-layout ranking-layout-stacked">
              <div className="top-ranking top-ranking-tiered">
                {artists.slice(0, 3).map((artist, index) => (
                  ...
                  >
                    {index === 0 ? (
                      <img src={artist.image} alt="" aria-hidden="true" loading="lazy" decoding="async" className="ranking-podium-fill" />
                    ) : null}
                    <div className="ranking-podium-content">
                      <span className={`ranking-medal ranking-medal-${index + 1}`}>
                        <span className="ranking-medal-icon">
                      ...
                      <img src={artist.image} alt={artist.name} loading="lazy" decoding="async" className="ranking-avatar" />
                      <h3>{artist.name}</h3>
              ...
              <div className="new-artists-grid">
                {newArtists.map((artist) => (
                  <article key={artist.name} className="new-artist-card">
                    <img src={artist.image} alt={artist.name} loading="lazy" decoding="async" className="new-artist-avatar" />
                    <h4>{artist.name}</h4>
